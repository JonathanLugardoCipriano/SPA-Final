...
<head>
    ...
    @if (file_exists(public_path('build/manifest.json')) || file_exists(public_path('hot')))
        ...
        @vite('resources/css/gimnasio/gimnasio_qr_code_styles.css')
...
<body>
    ...
    <main>
        <div class="main-container">
            <!-- Paso inicial: QR + Botón -->
            <div class="step active" id="step-qr">
                <div class="header">
                    <h2>Acceso al Gimnasio</h2>
                    <p style="text-align: center; text-wrap: wrap;">Escanea el código QR con tu celular o completa el
                        registro desde aquí</p>
                </div>

                <div class="qr-container" style="display: flex; flex-direction: column; align-items: center;">
                    <div class="qr-code">
                        <img id="qr-image" src="{{ route('gimnasio.qr-image', ['url' => $urlMovil]) }}&t={{ time() }}" alt="{{ $urlMovil }}">
                    </div>
                    {{-- <div class="qr-url">
                        <small id="qr-url" style="text-align: center; text-wrap: wrap;">{{ $urlMovil }}</small>
                    </div> --}}
                </div>

                <div class="access-options"
                    style="display: flex; justify-content: center; align-items: center; margin-top: 1rem;">
                    <button class="btn" onclick="showFormSelection()">
                        Completar desde aquí
                    </button>
                </div>
            </div>

            @include('gimnasio.componentes.formulario', ['startActive' => false, 'showBackToQR' => true])

            <div style="margin: 2rem 0; text-align: center;">
                <a href="{{ route('gimnasio.reglamento') }}" target="_blank">
                    Ver Reglamento del Gimnasio <i class="fas fa-external-link"></i>
                </a>
            </div>
        </div>
    </main>
    ...
    <script>
        ...
        function showQRStep() {
            ...
            resetForms(); // Limpiar todo cuando regresa al QR
        }
        ...
        function resetForms() {
            // Limpiar formularios
            if (document.getElementById('form-adult')) {
                document.getElementById('form-adult').reset();
            }
            if (document.getElementById('form-minor')) {
                document.getElementById('form-minor').reset();
            }

            // Resetear contador y limpiar menores adicionales
            resetMenoresContainer();
            contadorMenores = 1;

            // Limpiar firmas
            Object.keys(signaturesCanvas).forEach(canvasId => {
                if (signaturesCanvas[canvasId]) {
    ...
    <!-- Verificación y renovación del QR -->
    <script>
        // Configuración inicial
        const tokenInterno = '{{ $tokenInterno }}';
        let intervaloVerificacion;

        // Función para verificar y renovar el QR si es necesario
        async function verificarYRenovarQR() {
            try {
                const response = await fetch(`/gimnasio/verificar-renovar-qr/${tokenInterno}`, {
                    method: 'POST',
                    headers: {
                ...
                console.log('Respuesta de verificación/renovación:', data);

                // Si necesita renovación, actualizar la URL
                if (data.renovado) {
                    ...
                    // if (urlElement) {
                    //     urlElement.textContent = nuevaUrl;
                    // }

                    // Actualizar imagen QR
                    qrImage.src = `/gimnasio/qr-image?url=${encodeURIComponent(nuevaUrl)}&t=${Date.now()}`;
                    console.log('QR renovado exitosamente');
                }
            } catch (error) {
                console.error('Error al verificar/renovar QR:', error);
            }
        }
        ...
        function iniciarVerificacionPeriodica() {
            ...
            // Verificar cada 5 minutos (300,000 ms)
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // este es la url desde el controlador
            // const url_movil = @json($urlMovil);

            // console.log('URL móvil:', url_movil);
            // console.log('URL móvil codificada:', encodeURIComponent(url_movil));

            // const qrImage = document.getElementById('qr-image');
            // qrImage.src = `/gimnasio/qr-image?url=${encodeURIComponent(url_movil)}&t=${Date.now()}`;
        });
